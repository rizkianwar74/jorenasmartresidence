...
class AuthRepository {
  ...
  static Future<bool> tryRestoreSession() async {
    // Ambil user dari Firebase Auth (sudah tersedia setelah Firebase.initializeApp()).
    // Beri jeda singkat sebagai fallback untuk cold-start di beberapa device Android.
    ...
    // Coba refresh token — skip jika offline (tangkap semua error, jangan hang).
    try {
      await firebaseUser
          .reload()
          .timeout(const Duration(seconds: 6), onTimeout: () {});
    } on FirebaseAuthException catch (e) {
      // Akun dihapus atau disabled — paksa logout
      if (e.code == 'user-not-found' ||
          e.code == 'user-disabled' ||
          e.code == 'invalid-user-token') {
        return false;
      }
      // network-request-failed atau error lain → lanjut dengan sesi cache
    } catch (_) {
      // Timeout, SocketException, dll → lanjut
    }

    // Fetch data Firestore — coba server dulu, fallback ke cache lokal.
    // Ini penting agar sesi tetap pulih meski tidak ada koneksi internet.
    ...
    try {
      ...
      // Offline / timeout → coba dari cache lokal Firestore
      try {
        doc = await FirebaseFirestore.instance
            .collection('users')
            .doc(firebaseUser.uid)
        ...
        // Cache juga kosong (instalasi baru / cache dihapus) — tidak bisa restore
        return false;
      }
    }
  ...
  static Future<String?> register({
    ...
    try {
      ...
      try {
        ...
        // Rollback: hapus akun Auth yang baru dibuat supaya tidak jadi akun
        // "hantu" (ada di Firebase Auth tapi profilnya tidak pernah
        // tersimpan di Firestore). Tanpa rollback ini, percobaan registrasi
        // berikutnya dengan email yang sama akan selalu ditolak
        // "email-already-in-use" padahal user tidak pernah benar-benar
        // berhasil mendaftar.
        try { await credential.user?.delete(); } catch (_) {}
        return 'Gagal menyimpan data akun. Coba lagi.';
      }
